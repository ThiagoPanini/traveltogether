...
function row(trajeto: Trajeto) {
  const onAddResearch = vi.fn();
  return render(
    <ol>
      <TrajetoRow trajeto={trajeto} onAddResearch={onAddResearch} />
    </ol>,
  );
}

describe("TrajetoRow", () => {
  it("sua ida com proposta: rota, kicker, pílula e CTA de Pesquisa acionável", () => {
    const onAddResearch = vi.fn();
    render(
      <ol>
        <TrajetoRow
    ...
    const cta = screen.getByRole("button", { name: /registrar pesquisa/i });
    fireEvent.click(cta);
    expect(onAddResearch).toHaveBeenCalledOnce();
  ...
  it("volta-semente: faz a volta emergir com CTA próprio de Pesquisa", () => {
    ...
    expect(within(item).getByText(/a volta começa aqui/i)).toBeInTheDocument();
    expect(within(item).getByRole("button", { name: /registrar pesquisa/i })).toBeInTheDocument();
  });
});
