...
- **`takeoverIn` + `panelIn`** — `apps/web/components/fare-research.module.css`: o takeover da Pesquisa entra por opacidade e o conteúdo da prancheta por uma elevação curta; nenhuma informação depende da animação. Sob `prefers-reduced-motion: reduce`, ambas são desligadas e as transições dos controles principais são removidas.
