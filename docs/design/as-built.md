...
`apps/web/app/app/page.tsx` (server) + `pending-invitations.tsx` (client) + `actions.ts` (Server Action) + `app.module.css`. É a home do usuário dentro da área autenticada, concebida como uma **torre de controle doméstica**: menu lateral sticky com marca, navegação por âncoras, CTA "Nova viagem", Perfil e logout; no mobile ele se compacta em uma barra superior só com ícones, contadores e ações, sem retirar o logout. A Server Action `logout` revoga primeiro a sessão na API (`POST /auth/logout`, com o `Bearer` ainda vivo) e só depois limpa o cookie do Auth.js (`signOut({ redirectTo: "/entrar" })`.

O conteúdo usa somente dados reais de `GET /auth/me`, `GET /trips` e `GET /invitations`: herói com saudação e radar esquemático, métricas agregadas de Viagens, Paradas, papel Organizador e Convites, chamada acionável para aceitar Convites e uma grade de cartões de bordo para as Participações. Cada cartão mostra origem do Perfil → destino, nome, número de Paradas e papel, e leva ao Painel da Viagem. O primeiro cartão ocupa a largura da grade como ênfase visual, sem alegar prioridade, recência ou status que o backend não fornece. O radar é abstrato e decorativo; não promete geografia real e não introduz IATA.

Sem Viagens, o painel mantém um estado vazio honesto e substancial: manifesto `00`, CTA para o wizard e três passos que explicam Destino, Paradas e Tripulação. O caderno lateral reforça os invariantes de domínio — origem pertence ao Perfil e Preferida é por-pessoa — sem dinheiro, pontos, votação ou cascas globais "em breve".
...
`apps/web/app/app/viagens/[id]/page.tsx` (server) + `FareResearchTimeline` (client) + `panel.module.css`. É a **home de uma viagem** — onde se cai após criar. Lê `GET /trips/{id}` via `apiFetch()` (BFF); 404 → `notFound()` (não vaza existência — ADR-0011). O backbone continua vindo apenas da API real; a interface de exploração adiciona Pesquisas locais, explicitamente guardadas no navegador enquanto a API de Rota/Trecho/Pesquisa não existe. Não inventa voto, decisão coletiva ou conversão de unidade: o Painel mostra apenas que dinheiro e/ou pontos foram registrados, e os valores aparecem dentro do takeover da própria Pesquisa.

Anatomia: **header** (`Wordmark` + link "← Minhas viagens" com a seta `aria-hidden` + `nav` de seções com `TabChip` — Painel ativo `aria-current` e Roteiro/Orçamento/Ingressos como cascas "em breve" não-focáveis) · **herói** (eyebrow só com a partida "parte 14 set 2026" ou "datas a definir"; `h1` com o nome; subtítulo = paradas em ordem · "N viajantes"; contagem "dias p/ embarque" null-safe via `departureCountdown`, escondida sem data) · **progress strip** (`ProgressStrip`, mede os translados **compartilhados** propostos — "X de Y · Z%" + contador "N em discussão"; escondida quando não há salto compartilhado) · **grade 1.5fr/1fr**: à esquerda **Translados & Pesquisas**, à direita o **rail** (Tripulação + cascas "em breve") · **bottom switcher** fixo (`nav` "Vistas da viagem": Painel ativo + link funcional para `#pesquisas`). Stack em coluna única abaixo de 720px.

A **linha do tempo** (`ol` de `TrajetoRow`) é derivada por helper puro `deriveTrajetos` (`lib/trips/backbone.ts`): **sua ida** (casa→1ª parada, `entry_transfer`, por-pessoa) + os **compartilhados** (parada→parada, `desired_transfer`) + **sua volta-semente** (destino→casa, não modelada na criação). Cada linha mantém a rota, o kicker por-pessoa e a `StatusPill`, mas agora oferece o CTA real **Registrar pesquisa**. A volta deixa de ser beco sem saída: o mesmo CTA permite registrar um item só de volta ou ida-e-volta. Fichas já registradas aparecem sob o Trajeto com tipo, cobertura, fornecedor, data, dimensões de preço sem valores, link externo, edição e remoção; o storage é versionado e isolado por Viagem.

## Wizard de Pesquisa de translado

`FareResearchWizard` + `fare-research.module.css` implementam um takeover de quatro passos — **Translado → Detalhes → Valor → Revisar** — com estética de prancheta de despacho e um canhoto claro sempre vivo como resumo. O passo 1 parte do Trajeto e apenas semeia o tipo proposto, permitindo comparar outra modalidade; oferece só ida ou ida-e-volta e explicita que dois Trechos continuam sendo um item com preço registrado uma vez. O passo 2 adapta a ficha: avião exige IATA por Trecho e oferece Escala como campo do bilhete; terrestre não recebe campos aéreos. O passo 3 registra dinheiro e/ou pontos em dimensões separadas, preço por pessoa ou por veículo/serviço e notas, sem conversão nem ranking. O passo 4 revisa e salva localmente; uma ficha existente reabre no mesmo wizard para edição.

O modelo de UI vive em `lib/trips/fare-research.ts`: tipos cotáveis excluem a pé/carro próprio, ida-e-volta vira uma Pesquisa com dois Trechos, formatação preserva moeda/programa nativos e validações acontecem por passo. `localStorage` é uma decisão de protótipo funcional desta superfície, não substituto silencioso da futura API; a copy diz isso no rodapé, na revisão e junto das fichas.
...
Componentes reutilizáveis novos (em `apps/web/components/`, cada um com CSS module + teste Vitest, pensados para reuso na futura tela Rotas): `StatusPill` (tom = texto+cor+borda, nunca só cor), `ProgressStrip` (`role="progressbar"`, sem animar largura), `TabChip` (ativo `aria-current` / "em breve" `aria-disabled` não-focável com sufixo `.sr-only`), `EmBreveCard` (borda tracejada, ícone lucide decorativo), `CrewRow` (variante `blind`), `TrajetoRow`, `FareResearchTimeline` e `FareResearchWizard`. As derivações puras de backbone moram em `lib/trips/backbone.ts`; o modelo/validação/persistência das fichas mora em `lib/trips/fare-research.ts`; ambos são testados sem rede.
